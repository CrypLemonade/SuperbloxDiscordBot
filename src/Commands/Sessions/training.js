...
module.exports = {
        data: new SlashCommandBuilder()
                .setName('training')
                .setDescription('hosts a training')
        ...
        async execute(interaction) {
                ...
                if (interaction.member.roles.cache.some(role => role.id === '1208282949624275036')) {
                        const trainingEmbed = new EmbedBuilder()
                                ...
                                .setDescription('```A shift is starting soon. Please report to the Session Allocation Room once you have attired the uniform. Although attendance is not mandatory, we encourage all employees to join us in ensuring that our store runs efficiently.```')
                                ...
                                .addFields({
                                        name: 'Game Link:',
                                        value: 'https://www.roblox.com/games/8675322338/Training-Center#!/about',
                                        inline: false
                                })
                                .setFooter({ text: 'Superblox Official Bot - 2024' })
                                .setTimestamp()

                        await interaction.reply({ content: '@everyone', allowedMentions: { parse: ["everyone"] }, embeds: [trainingEmbed] });
                } else {
